/* ==========================================================================
   ...
   Handles: icon fallback, mandatory-field validation, PIN/QR generation,
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------------------------------------------------------------------
     0. Icon fallback — every <img> is left with an empty src so real
        icons can be dropped in later. Until then, hide the broken image
        and show a soft placeholder dot instead.
  --------------------------------------------------------------------- */
  document.querySelectorAll('.field-icon img').forEach(img => {
    const markMissing = () => img.closest('.field-icon').classList.add('no-icon');
