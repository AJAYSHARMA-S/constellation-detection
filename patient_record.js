document.addEventListener('DOMContentLoaded', () => {
  ...
  const consentCard   = consentCheckbox ? consentCheckbox.closest('.consent') : null;
  ...
  function validateForm() {
    ...
    reportsList.querySelectorAll('.report-item').forEach((item) => {
      ...
      nameGroup.classList.toggle('has-error', nameEmpty);
      fileGroup.classList.toggle('has-error', fileEmpty);
  ...
  if (recordForm) {
    recordForm.addEventListener('submit', (e) => {
      ...
      if (!validateForm()) {
        const firstError = recordForm.querySelector('.has-error, .terms-box.has-error');
        if (firstError) {
  ...
  if (newRecordBtn) {
    newRecordBtn.addEventListener('click', () => {
      ...
      termsBox.classList.remove('has-error');
      consentCard.classList.remove('has-error');
      recordForm.querySelectorAll('.form-group.has-error').forEach((g) => g.classList.remove('has-error'));
